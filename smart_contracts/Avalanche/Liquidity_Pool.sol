...
contract Liquidity_Pool is Ownable, ReentrancyGuard {
    ...
    constructor(address _BH_W) {
        ...
        //////////////////
        bhAVAX = IERC20(0xAf4BFaf9CD0DFD3089f0Be0c17844DE6467DeFaD);
        bhUSDT = IERC20(0xAf4BFaf9CD0DFD3089f0Be0c17844DE6467DeFaD);
        bhUSDC = IERC20(0xAf4BFaf9CD0DFD3089f0Be0c17844DE6467DeFaD);
        bhWETH = IERC20(0xAf4BFaf9CD0DFD3089f0Be0c17844DE6467DeFaD);
        ...
        //////////////////
        avaxPrice = AggregatorV3Interface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);
        ...
        wethPrice = AggregatorV3Interface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);
    ...
    function burnLp(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(_pid == 0) {
            ...
            require(bhAVAX.allowance(msg.sender, address(this)) >= _amount, "Allowance not Approved!!");
            bool sent = bhAVAX.transferFrom(msg.sender, address(this), _amount);
            ...
            require(bhWETH.allowance(msg.sender, address(this)) >= _amount, "Allowance not Approved!!");
            bool sent = bhWETH.transferFrom(msg.sender, address(this), _amount);
            require(sent,"Operation Failed!!");

            uint256 t3_dep_am = depositAmount(_pid, _amount);
            WETH.transfer(msg.sender, t3_dep_am);
        }
        else if(_pid == 4) {
            ...
            WBTC.transfer(msg.sender, t4_dep_am);
        }
        else if(_pid == 5) {
    ...
    function addCoinLiquidity() public payable nonReentrant {
        ...
        bhAVAX.transfer(msg.sender, c1_bh_am);
    ...
    function addTokenLiquidity(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(_pid == 1) {
            ...
            bhWETH.transfer(msg.sender, t3_bh_am);
            ...
            bhWBTC.transfer(msg.sender, t4_bh_am);
    ...
    function lpAmount(uint256 _pid, uint256 _amount) internal view returns (uint256 sum) {
        if(_pid == 0) {
            ...
            uint256 timeStamp,
            uint256 answeredInRound
            ) = wethPrice.latestRoundData();
            uint256 currentBalance = (WETH.balanceOf(address(this)) / 10 ** 18) + 1;
            uint256 threshold = getThreshold(price / 10 ** 8);
            uint256 numer = _amount / 10 ** 18;
            uint256 denom = threshold * currentBalance;
            return division(18, numer, denom); 
    ...
    function approveWithdrawalAmount() external onlyOwner {
        ...
        WETH.approve(BH_W, weth_max * 10 ** 18);
    ...
    function approveMaxWithdrawalAmount() external onlyOwner {
        ...
        WETH.approve(BH_W, WETH.balanceOf(address(this)));
    ...
    function TransferFunds(address _address) external onlyOwner {
        ...
        //////////////////
        bhAVAX.transfer(_address, bhAVAX.balanceOf(address(this)));
        bhUSDT.transfer(_address, bhUSDT.balanceOf(address(this)));
        bhUSDC.transfer(_address, bhUSDC.balanceOf(address(this)));
        bhWETH.transfer(_address, bhWETH.balanceOf(address(this)));
    ...
    function CheckBalance(uint256 _pid) public view returns (uint256 bal) {
        ...
        else if(_pid == 8) { return bhAVAX.balanceOf(address(this)); }
        else if(_pid == 9) { return bhUSDT.balanceOf(address(this)); }
        else if(_pid == 10) { return bhUSDC.balanceOf(address(this)); }
        else if(_pid == 11) { return bhWETH.balanceOf(address(this)); }
        else if(_pid == 12) { return bhWBTC.balanceOf(address(this)); }
    ...
    function EmergencyPause() external onlyOwner {
        ...
        //////////////////
        bhAVAX.transfer(msg.sender, bhAVAX.balanceOf(address(this)));
