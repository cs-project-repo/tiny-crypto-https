...
contract Contract_1 is Ownable, ReentrancyGuard {
    ...
    function depositToken(uint256 _pid, uint256 _amount, bool _split) external nonReentrant {
        ...
        if(_pid == 1) {
            ...
            uint256 tx_fee_1 = _amount * fee_1;
            uint256 tx_rest = _amount * (100 - fee_1);
            BTC.transfer(SC_2, tx_fee_1/100);
            BTC.transfer(BH_LP, tx_rest/100);
            }
            else {
                uint256 tx_fee_2 = _amount * fee_2;
                uint256 tx_rest = _amount * (100 - fee_2);
                BTC.transfer(SC_2, tx_fee_2/100);
                BTC.transfer(BH_LP, tx_rest/100);
            } 
        }
        else if(_pid == 4) {
            ...
            uint256 tx_fee_1 = _amount * fee_1;
            uint256 tx_rest = _amount * (100 - fee_1);
            BNB.transfer(SC_2, tx_fee_1/100);
            BNB.transfer(BH_LP, tx_rest/100);
            }
            else {
                uint256 tx_fee_2 = _amount * fee_2;
                uint256 tx_rest = _amount * (100 - fee_2);
                BNB.transfer(SC_2, tx_fee_2/100);
                BNB.transfer(BH_LP, tx_rest/100);
            } 
        }
        else if(_pid == 5) {
    ...
    function CheckAllowance(uint256 _pid) public view returns (uint256) {
        require(_pid >= 1 || _pid <= 6, "Please Select Correct Token!!");
        if(_pid == 1){ return USDC.allowance(msg.sender,address(this)); }
        else if(_pid == 2){ return ETH.allowance(msg.sender,address(this)); }
        else if(_pid == 3){ return BTC.allowance(msg.sender,address(this)); }
        else if(_pid == 4){ return BNB.allowance(msg.sender,address(this)); }
