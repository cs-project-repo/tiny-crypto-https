...
contract Withdraw is ReentrancyGuard, ChainlinkClient, ConfirmedOwner {
    ...
    function fulfill(bytes32 _requestId, bool _proof) public recordChainlinkFulfillment(_requestId) {
        ...
        if(_pid == 0) {
            ...
            ETH.transferFrom(BH_LP, _withdrawalAddress, _amount);
        }
        else if(_pid == 4) {
            MATIC.transferFrom(BH_LP, _withdrawalAddress, _amount);
        }
        else if(_pid == 5) {
            AVAX.transferFrom(BH_LP, _withdrawalAddress, _amount);
        }
        else if(_pid == 6) {
